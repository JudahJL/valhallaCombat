...
namespace Utils {
    ...
    namespace AvOwner {
        // yoinked from https://github.com/D7ry/CommonLibSSE-NG/blob/88161ebbbd95331bca794cdb9f5f6fa34515793d/include/REL/Relocation.h#L2159
        template<class T, class This>
        [[nodiscard]] inline T& RelocateParentIfNewer(REL::Version v, This* a_self, std::ptrdiff_t older, std::ptrdiff_t newer) {
        ...
        inline RE::Actor* asActor(RE::ActorValueOwner* a_avOwner) {
            return &RelocateParentIfNewer<RE::Actor>(SKSE::RUNTIME_SSE_1_6_629, a_avOwner, 0xB0, 0xB8);
        }
    }  // namespace AvOwner
}  // namespace Utils
...
namespace inlineUtils {
    ...
    inline void slowTime(float a_duration, float a_percentage) {
        int duration_milisec = static_cast<int>(a_duration * 1'000);
...
namespace TrueHUDUtils {
    ...
    inline void greyoutAvMeter(RE::Actor* a_actor, RE::ActorValue actorValue) {
        ...
        ersh->OverrideBarColor(a_actor->GetHandle(), actorValue, TRUEHUD_API::BarColorType::FlashColor, 0xd7'2a'2a);
        ersh->OverrideBarColor(a_actor->GetHandle(), actorValue, TRUEHUD_API::BarColorType::BarColor, 0x7d'7e'7d);
        ersh->OverrideBarColor(a_actor->GetHandle(), actorValue, TRUEHUD_API::BarColorType::PhantomColor, 0xb3'0d'10);
    }
    ...
    inline void greyOutSpecialMeter(RE::Actor* a_actor) {
        ...
        auto ersh = ValhallaCombat::GetSingleton()->ersh_TrueHUD;
        ersh->OverrideSpecialBarColor(a_actor->GetHandle(), TRUEHUD_API::BarColorType::FlashColor, 0xd7'2a'2a);
        ersh->OverrideSpecialBarColor(a_actor->GetHandle(), TRUEHUD_API::BarColorType::BarColor, 0x7d'7e'7d);
        ersh->OverrideSpecialBarColor(a_actor->GetHandle(), TRUEHUD_API::BarColorType::PhantomColor, 0xb3'0d'10);
        ersh->OverrideBarColor(a_actor->GetHandle(), RE::ActorValue::kHealth, TRUEHUD_API::BarColorType::FlashColor, 0xd7'2a'2a);
    }
...
#pragma endregion
    ...
    static void clampDmg(float& dmg, RE::Actor* aggressor) {
        ...
        if(a_weapon) {
            ...
            dmg = std::min(dmg, static_cast<float>(a_weapon->GetAttackDamage()));
        }
    }
    ...
    static void resetProjectileOwner(RE::Projectile* a_projectile, RE::Actor* a_actor, RE::hkpCollidable* a_projectile_collidable) {
        ...
        a_projectile_collidable->broadPhaseHandle.collisionFilterInfo &= (0x00'00'FF'FF);
    ...
    public:
        formLoader(RE::BSFixedString pluginName) {
            ...
            if(!_dataHandler->LookupModByName(pluginName)) {
                logger::critical("Error: {} not found.", pluginName.data());
            }
            logger::info("Loading from plugin {}...", pluginName.data());
        }

        ~formLoader() {
            logger::info("Loaded {} forms from {}", _loadedForms, _pluginName.data());
        }
        ...
        void load(formType*& formRet, RE::FormID formID) {
            ...
            if(!formRet) {
                logger::critical("Error: null formID or wrong form type when loading {} from {}", formID, _pluginName.data());
            }
            _loadedForms++;
        }
    };
