...
#pragma region projectileHit
    ...
    /*Decide whether the collision is an actor-projectile collision. If it is, initialize a deflection attempt by the actor.
...
#pragma endregion
    ...
    static void unblock_delayed_threadfunc(RE::AttackBlockHandler* a_this, RE::ButtonEvent* a_event, RE::PlayerControlsData* a_data, float a_time) {
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(a_time * 1'000)));  // wait for, by default, 0.3 sec
        std::thread(
