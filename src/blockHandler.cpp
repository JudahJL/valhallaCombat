...
bool blockHandler::isInBlockAngle(RE::Actor* blocker, RE::TESObjectREFR* a_obj) {
    auto angle = blocker->GetHeadingAngle(a_obj->GetAngle(), true);
...
bool blockHandler::processProjectileBlock(RE::Actor* a_blocker, RE::Projectile* a_projectile, RE::hkpCollidable* a_projectile_collidable) {
    ...
    if((isInBlockAngle(a_blocker, a_projectile) && a_blocker->IsBlocking()) || forced) {
        // evaluate cost
        float cost{};
        ...
        if(a_projectile->GetProjectileRuntimeData().spell) {  // parry spell
            ...
            if(const auto launcher = a_projectile->GetProjectileRuntimeData().weaponSource) {
                cost += launcher->GetAttackDamage();
            }
            if(const auto ammo = a_projectile->GetProjectileRuntimeData().ammoSource) {
                cost += ammo->GetRuntimeData().data.damage;
            }
        }
...
PRECISION_API::PreHitCallbackReturn blockHandler::precisionPrehitCallbackFunc(const PRECISION_API::PrecisionHitData& a_precisionHitData) {
    ...
    if(settings::bTimedBlockToggle && blockHandler::GetSingleton()
    ...
    if(settings::bTackleToggle && targetActor->AsActorState()->GetAttackState() != RE::ATTACK_STATE_ENUM::kNone) {
...
void blockHandler::playBlockEffects(RE::Actor* blocker, RE::Actor* attacker, blockType blockType) {
    ...
    if(settings::bTimedBlockScreenShake && (blocker->IsPlayerRef() || (attacker && attacker->IsPlayerRef()))) {
        playBlockScreenShake(blocker, blockType);
    }

    if(settings::bTimedBlockSlowTime && ((attacker && attacker->IsPlayerRef()) || blocker->IsPlayerRef())) {
...
void blockHandler::EldenCounterCompatibility::attemptInit() {
    ...
    ec_triggerSpell = RE::TESDataHandler::GetSingleton()->LookupForm<RE::SpellItem>(0x8'01, "EldenCounter.esp");
    if(!ec_triggerSpell) {
...
void blockHandler::EldenCounterCompatibility::async_removeECTriggerSpell(RE::Actor* a_actor) {
    std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(ec_Time * 1'000)));
    if(!a_actor) {
        return;
    }
