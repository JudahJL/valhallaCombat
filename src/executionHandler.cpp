...
bool executionHandler::attemptExecute(RE::Actor* a_executor, RE::Actor* a_victim) {
    ...
    if(!settings::bStunToggle || !Utils::Actor::isHumanoid(a_executor) || a_executor->IsDead() || a_victim->IsDead() || !a_executor->Is3DLoaded() || !a_victim->Is3DLoaded() || a_executor->IsInKillMove() || a_victim->IsInKillMove() || a_executor->IsOnMount() || a_victim->IsOnMount() || a_victim->IsPlayerRef() || a_victim->IsPlayerTeammate() || a_victim->IsEssential() || a_victim->IsInKillMove() || a_victim->AsMagicTarget()->HasEffectWithArchetype(RE::MagicTarget::Archetype::kParalysis)) {
