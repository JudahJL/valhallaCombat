...
static inline std::unordered_map<RE::TESRace*, float> staminaRegenMap;
...
void settings::updateGameSettings() {
    ...
    if(RE::Setting* bVal_TrueHudAPI_Acq = gsc->GetSetting("bVal_TrueHudAPI_Acq")) {
        bVal_TrueHudAPI_Acq->data.b = ValhallaCombat::GetSingleton()->ersh_TrueHUD != nullptr;
    }
    if(RE::Setting* bVal_TrueHUDAPI_SpecialMeter_Acq = gsc->GetSetting("bVal_TrueHUDAPI_SpecialMeter_Acq")) {
